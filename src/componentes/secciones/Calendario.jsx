  ...
  const onSubmit = (data) => {
    ...
    if (data.fecha_inicio && data.fecha_fin) {
      ...
      for (let i = 0; i < dates.length; i++) {
        data.precio_doble &&
          registrar(data.precio_doble, dates[i], "hotpink", "doble");
        data.precio_triple &&
          registrar(data.precio_triple, dates[i], "#2cabce", "triple");
        data.precio_individual &&
          registrar(data.precio_individual, dates[i], "#a856c9", "individual");
      }
    } else if (data.fecha_inicio && !data.fecha_fin) {
      data.precio_doble &&
        registrar(data.precio_doble, data.fecha_inicio, "hotpink", "doble");
      data.precio_triple &&
        registrar(data.precio_triple, data.fecha_inicio, "#2cabce", "triple");
      data.precio_individual &&
        registrar(
          data.precio_individual,
          data.fecha_inicio,
  ...
  function deshabilitar(data) {
    ...
    if (data.fecha_inicio_deshabilitar) {
      if (data.fecha_inicio_deshabilitar && !data.fecha_fin_deshabilitar) {
        ...
        data.fecha_inicio_deshabilitar &&
        data.fecha_fin_deshabilitar
      ) {
    ...
    <div className=" text-black  dark:text-white">
      ...
      <div className="flex flex-row justify-between md:justify-end col-end space-x-3 ">
        <div className="flex w-fit">
          <div className="h-[20px] w-[20px] rounded-full bg-purple-500 mr-3"></div>
          <span className="text-sm ">Individual</span>
        </div>
        <div className="flex w-fit">
          <div className="h-[20px] w-[20px] rounded-full bg-pink-500 mr-3"></div>
          <span className="text-sm ">Doble</span>
        </div>
        <div className="flex w-fit">
          <div className="h-[20px] w-[20px] rounded-full bg-cyan-500 mr-3"></div>
      ...
      <div className="mb-4 border-b border-gray-200 dark:border-gray-700">
        <ul
          ...
          data-tabs-active-classes="dark:text-cyan-400 text-orange-400 hover:text-orange-500 dark:text-cyan-400 dark:hover:text-cyan-300 border-orange-400 dark:border-cyan-400"
          data-tabs-inactive-classes="dark:border-transparent text-gray-500 hover:text-orange-400 dark:text-gray-400 border-transparent hover:border-gray-200 dark:border-gray-700 dark:hover:text-gray-300"
          role="tablist"
        >
